#-------------------------------------------------------------------------------
# File: greedy.jl
# Description: This file contains functions used to construct a greedy solution.
#   inspired by work of
...
function greedy(instance::Instance)
    ...
    for position in (instance.nb_late_prec_day+1):(instance.nb_cars)
        ...
        candidates = criterion_nb_violations(solution, position, V, instance, 1,instance.nb_HPRC)
        ...
        if length(candidates) > 1
            candidates = criterion_tie_break(candidates, instance, rv, rpi, length_pi)
        end
        ...
        if length(candidates) > 1
            ...
            candidates = criterion_second_tie_break(candidates, instance, rv, rpi)
        end
