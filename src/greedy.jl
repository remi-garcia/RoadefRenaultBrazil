...
function update_solution!(   solution::Solution,
...
function update_sol_atpos!( solution::Solution,
                            nb::Int,
                            pos::Int,
                            p::Array{Int, 1}, q::Array{Int, 1},
                            flag::Array{Bool, 2},
...
function greedy(inst::Instances)
    ...
    for pos in (inst.nb_late_prec_day+1):(solution.n)
        ...
        # Update M1, M2 and M3
        update_sol_atpos!(solution, inst.nb_HPRC, pos, inst.HPRC_p, inst.HPRC_q, inst.HPRC_flag)
        update_sol_atpos!(solution, inst.nb_LPRC, pos, inst.LPRC_p, inst.LPRC_q, inst.LPRC_flag, inst.nb_HPRC)
    end
